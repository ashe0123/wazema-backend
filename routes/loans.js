...
async function totalSaved(memberId) {
  const r = await db.one("SELECT COALESCE(SUM(amount),0) as t FROM savings WHERE member_id=? AND status IN ('paid','late')", [memberId]);
  return Number(r.t);
}
async function withRepayments(loans) {
  return Promise.all(loans.map(async l => ({
    ...l,
    repayments: await db.all('SELECT * FROM repayments WHERE loan_id=? ORDER BY month ASC', [l.id])
  })));
}
...
router.get('/', authMiddleware, async (req, res, next) => {
  try {
    if (req.user.role === 'member') {
      const loans = await db.all('SELECT * FROM loans WHERE member_id=? ORDER BY request_date DESC', [req.user.id]);
    ...
    const p = [];
    if (status) { sql += ' WHERE l.status=?'; p.push(status); }
    sql += ` ORDER BY l.request_date DESC LIMIT ? OFFSET ?`;
    p.push(limit, offset);
    const rows = await withRepayments(await db.all(sql, p));
    if (!req.query.page && !req.query.limit) return res.json(rows);
    const countSql = status
      ? 'SELECT COUNT(*) as c FROM loans WHERE status=?'
...
router.get('/:id/eligibility', authMiddleware, async (req, res, next) => {
  try {
    ...
    const monthsRow = await db.one("SELECT COUNT(*) as c FROM savings WHERE member_id=? AND status IN ('paid','late')", [req.params.id]);
    const monthsPaid = Number(monthsRow.c);
    const activeLoan = await db.one("SELECT id FROM loans WHERE member_id=? AND status IN ('active','pending')", [req.params.id]);
...
router.get('/:id', authMiddleware, async (req, res, next) => {
  try {
    const loan = await db.one('SELECT * FROM loans WHERE id=?', [req.params.id]);
    if (!loan) return res.status(404).json({ error: 'Loan not found' });
    if (req.user.role==='member' && loan.member_id!==req.user.id) return res.status(403).json({ error: 'Access denied' });
    loan.repayments = await db.all('SELECT * FROM repayments WHERE loan_id=? ORDER BY month ASC', [loan.id]);
    res.json(loan);
  } catch(e) { next(e); }
});
...
router.post('/', authMiddleware, async (req, res, next) => {
  try {
    ...
    const member = await db.one("SELECT * FROM members WHERE id=? AND status='active'", [memberId]);
    if (!member) return res.status(404).json({ error: 'Active member not found' });
    const existing = await db.one("SELECT id FROM loans WHERE member_id=? AND status IN ('active','pending')", [memberId]);
    ...
    const id = genId('LN');
    await db.run("INSERT INTO loans (id,member_id,amount,request_date,status,queue_position,admin_note) VALUES (?,?,?,CURRENT_DATE,'pending',?,?)",
...
router.post('/:id/approve', authMiddleware, adminOnly, async (req, res, next) => {
  try {
    ...
    await db.transaction(async (tx) => {
      await tx.run("UPDATE loans SET status='active',approve_date=?,third_party_ref=?,third_party_signed=1,queue_position=NULL,guarantor_name=?,guarantor_phone=?,third_party_doc_url=? WHERE id=?",
      ...
      for (let i = 0; i < months; i++) {
        await tx.run('INSERT INTO repayments (id,loan_id,month,amount,status) VALUES (?,?,?,?,?)',
      ...
      for (let i = 0; i < pending.length; i++) {
        await tx.run('UPDATE loans SET queue_position=? WHERE id=?', [i+1, pending[i].id]);
      }
    });
...
router.post('/:id/reject', authMiddleware, adminOnly, async (req, res, next) => {
  try {
    ...
    if (!loan) return res.status(404).json({ error: 'Loan not found' });
    if (loan.status !== 'pending') return res.status(400).json({ error: 'Loan is already ' + loan.status });
    await db.run("UPDATE loans SET status='rejected',rejection_reason=?,queue_position=NULL WHERE id=?", [rejection_reason.trim(), loan.id]);
    const pending = await db.all("SELECT id FROM loans WHERE status='pending' ORDER BY queue_position ASC,request_date ASC");
    for (let i = 0; i < pending.length; i++) await db.run('UPDATE loans SET queue_position=? WHERE id=?', [i+1, pending[i].id]);
...
router.post('/:id/disburse', authMiddleware, adminOnly, async (req, res, next) => {
  try {
    ...
    await db.run('UPDATE loans SET disbursement_date=? WHERE id=?', [disbursement_date, loan.id]);
...
router.patch('/:id/guarantor', authMiddleware, adminOnly, async (req, res, next) => {
  try {
    ...
    const loan = await db.one('SELECT * FROM loans WHERE id=?', [req.params.id]);
    if (!loan) return res.status(404).json({ error: 'Loan not found' });
    await db.run('UPDATE loans SET guarantor_name=?,guarantor_phone=? WHERE id=?', [guarantor_name.trim(), guarantor_phone||null, loan.id]);
...
router.post('/:id/refinance', authMiddleware, adminOnly, async (req, res, next) => {
  try {
    ...
    await db.transaction(async (tx) => {
      ...
      await tx.run("UPDATE repayments SET status='refinanced' WHERE loan_id=? AND status NOT IN ('paid')", [loan.id]);
      // Update loan amount
      await tx.run('UPDATE loans SET amount=?,admin_note=? WHERE id=?',
