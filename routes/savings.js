...
router.get('/summary', authMiddleware, adminOnly, async (req, res, next) => {
  try {
    ...
    const paid    = await db.all('SELECT * FROM savings WHERE month=?', [month]);
...
router.get('/yearly', authMiddleware, async (req, res, next) => {
  try {
    ...
    const p = [];
    if (memberId) { sql += ' AND member_id=?'; p.push(memberId); }
...
router.get('/interest', authMiddleware, async (req, res, next) => {
  try {
    ...
    const rows = await db.all('SELECT * FROM interest_accruals WHERE member_id=? ORDER BY month DESC', [memberId]);
...
router.post('/accrue-interest', authMiddleware, adminOnly, async (req, res, next) => {
  try {
    ...
    for (const m of members) {
      const existing = await db.one('SELECT id FROM interest_accruals WHERE member_id=? AND month=?', [m.id, month]);
      if (existing) { results.push({ member_id: m.id, status: 'already_accrued' }); continue; }
      const balRow = await db.one("SELECT COALESCE(SUM(amount),0) as t FROM savings WHERE member_id=? AND status IN ('paid','late')", [m.id]);
      ...
      await db.run('INSERT INTO interest_accruals (member_id,month,balance,rate,interest_amount) VALUES (?,?,?,?,?)',
...
router.get('/', authMiddleware, async (req, res, next) => {
  try {
    if (req.user.role === 'member')
      return res.json(await db.all('SELECT * FROM savings WHERE member_id=? ORDER BY month ASC', [req.user.id]));
    const { memberId, month } = req.query;
...
router.post('/', authMiddleware, async (req, res, next) => {
  try {
    ...
    const member = await db.one('SELECT id,monthly_saving FROM members WHERE id=?', [targetId]);
    ...
    const existing = await db.one('SELECT id FROM savings WHERE member_id=? AND month=?', [targetId, month]);
    ...
    const id = genId('SV');
    await db.run('INSERT INTO savings (id,member_id,month,amount,paid_date,status,penalty,bank_name,account_number) VALUES (?,?,?,?,?,?,?,?,?)',
...
router.post('/bulk-admin', authMiddleware, adminOnly, async (req, res, next) => {
  try {
    ...
    for (let i = 0; i < members.length; i++) {
      const m = members[i];
      const existing = await db.one('SELECT id FROM savings WHERE member_id=? AND month=?', [m.id, month]);
      if (existing) { results.push({ member_id: m.id, status: 'already_recorded' }); continue; }
      const id = genId('SV') + i;
      await db.run('INSERT INTO savings (id,member_id,month,amount,paid_date,status,penalty) VALUES (?,?,?,?,?,?,0)',
...
router.get('/:id', authMiddleware, async (req, res, next) => {
  try {
    const row = await db.one(
      'SELECT s.*,m.name as member_name,m.phone as member_phone,m.monthly_saving FROM savings s JOIN members m ON s.member_id=m.id WHERE s.id=?',
      [req.params.id]
    );
...
router.patch('/:id/confirm', authMiddleware, adminOnly, async (req, res, next) => {
  try {
    ...
    const row = await db.one('SELECT * FROM savings WHERE id=?', [req.params.id]);
    if (!row) return res.status(404).json({ error: 'Record not found' });
    await db.run('UPDATE savings SET status=?,penalty=? WHERE id=?',
...
router.post('/bulk-confirm', authMiddleware, adminOnly, async (req, res, next) => {
  try {
    ...
    for (const id of ids) {
      try {
        const row = await db.one('SELECT * FROM savings WHERE id=?', [id]);
        if (!row) {
        ...
        await db.run('UPDATE savings SET status=? WHERE id=?', [finalStatus, id]);
...
router.patch('/:id/waive-penalty', authMiddleware, adminOnly, async (req, res, next) => {
  try {
    const row = await db.one('SELECT * FROM savings WHERE id=?', [req.params.id]);
    if (!row) return res.status(404).json({ error: 'Record not found' });
    await db.run('UPDATE savings SET penalty=0 WHERE id=?', [req.params.id]);
