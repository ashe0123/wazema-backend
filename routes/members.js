...
async function auditLog(actor, action, target, detail) {
  try { await db.run('INSERT INTO audit_log (actor,action,target,detail) VALUES (?,?,?,?)', [actor, action, target||null, detail||null]); } catch {}
}
...
router.get('/', authMiddleware, async (req, res, next) => {
  try {
    if (req.user.role === 'admin') {
      ...
      const params = [];
      if (search) {
        sql += ' WHERE (name ILIKE ? OR id ILIKE ? OR phone ILIKE ?)';
        params.push(search, search, search);
      }
      sql += ` ORDER BY created_at ASC LIMIT ? OFFSET ?`;
      params.push(limit, offset);
      const countSql = search
        ? 'SELECT COUNT(*) as c FROM members WHERE (name ILIKE ? OR id ILIKE ? OR phone ILIKE ?)'
        : 'SELECT COUNT(*) as c FROM members';
      const [rows, total] = await Promise.all([
        db.all(sql, params),
        db.one(countSql, search ? [search, search, search] : []),
      ]);
    ...
    const m = await db.one('SELECT * FROM members WHERE id=?', [req.user.id]);
...
router.post('/', authMiddleware, adminOnly, async (req, res, next) => {
  try {
    ...
    if (!/^[a-zA-Z\s\u1200-\u137F]+$/.test(first_name.trim())) return res.status(400).json({ error: 'First name can only contain letters (Amharic/English)' });
    if (!middle_name?.trim()) return res.status(400).json({ error: 'Middle name is required' });
    if (!/^[a-zA-Z\s\u1200-\u137F]+$/.test(middle_name.trim())) return res.status(400).json({ error: 'Middle name can only contain letters (Amharic/English)' });
    if (!last_name?.trim())  return res.status(400).json({ error: 'Last name is required' });
    if (!/^[a-zA-Z\s\u1200-\u137F]+$/.test(last_name.trim())) return res.status(400).json({ error: 'Last name can only contain letters (Amharic/English)' });
    if (first_name.trim().length < 2) return res.status(400).json({ error: 'First name must be at least 2 characters' });
    if (first_name.trim().length > 50) return res.status(400).json({ error: 'First name is too long (maximum 50 characters)' });
    if (middle_name.trim().length < 2) return res.status(400).json({ error: 'Middle name must be at least 2 characters' });
    if (middle_name.trim().length > 50) return res.status(400).json({ error: 'Middle name is too long (maximum 50 characters)' });
    if (last_name.trim().length < 2) return res.status(400).json({ error: 'Last name must be at least 2 characters' });
    if (last_name.trim().length > 50) return res.status(400).json({ error: 'Last name is too long (maximum 50 characters)' });
    ...
    if (isNaN(monthly_saving) || Number(monthly_saving) < 10) return res.status(400).json({ error: 'Monthly saving must be at least 10 ETB' });
    if (Number(monthly_saving) > 100000) return res.status(400).json({ error: 'Monthly saving is too high (maximum 100,000 ETB)' });
    if (!join_date)          return res.status(400).json({ error: 'Join date is required' });
    if (!/^\d{4}-\d{2}-\d{2}$/.test(join_date)) return res.status(400).json({ error: 'Join date must be in format YYYY-MM-DD' });
    if (!date_of_birth)      return res.status(400).json({ error: 'Date of birth is required' });
    if (!/^\d{4}-\d{2}-\d{2}$/.test(date_of_birth)) return res.status(400).json({ error: 'Date of birth must be in format YYYY-MM-DD' });
    if (!address?.trim())    return res.status(400).json({ error: 'Address / Kebele is required' });
    if (address.trim().length < 3) return res.status(400).json({ error: 'Address must be at least 3 characters' });
    if (address.trim().length > 200) return res.status(400).json({ error: 'Address is too long (maximum 200 characters)' });
    if (!password?.trim())   return res.status(400).json({ error: 'Password is required' });
    if (password.length < 4) return res.status(400).json({ error: 'Password must be at least 4 characters' });
    if (password.length > 50) return res.status(400).json({ error: 'Password is too long (maximum 50 characters)' });
    if (!['Regular','Interest','Childrens'].includes(account_type))
      return res.status(400).json({ error: 'Invalid account type' });

    const normalizedPhone = normalizePhone(phone);
    const existing = await db.one('SELECT id FROM members WHERE phone=?', [normalizedPhone]);
    ...
    await db.run(
      ...
      [id, first_name.trim(), middle_name.trim(), last_name.trim(), name,
       ...
       'active', bcrypt.hashSync(password, 10)]
    );
    ...
    await db.run(
      'INSERT INTO savings (id,member_id,month,amount,paid_date,status,penalty) VALUES (?,?,?,?,?,?,0)',
...
router.patch('/:id', authMiddleware, adminOnly, async (req, res, next) => {
  try {
    ...
    if (first_name && !nameRegex.test(first_name.trim())) {
      return res.status(400).json({ error: 'First name can only contain letters (Amharic/English)' });
    }
    if (middle_name && !nameRegex.test(middle_name.trim())) {
      return res.status(400).json({ error: 'Middle name can only contain letters (Amharic/English)' });
    }
    if (last_name && !nameRegex.test(last_name.trim())) {
      return res.status(400).json({ error: 'Last name can only contain letters (Amharic/English)' });
    }
    ...
    await db.run(
      `UPDATE members SET first_name=?,middle_name=?,last_name=?,name=?,phone=?,email=?,
       monthly_saving=?,account_type=?,saving_interest_pct=?,share_amount=?,share_qty=?,
       registration_fee=?,share_paid=?,reg_fee_paid=? WHERE id=?`,
...
router.patch('/:id/status', authMiddleware, adminOnly, async (req, res, next) => {
  try {
    ...
    const r = await db.run('UPDATE members SET status=? WHERE id=?', [status, req.params.id]);
...
router.patch('/:id/reset-password', authMiddleware, adminOnly, async (req, res, next) => {
  try {
    ...
    const r = await db.run('UPDATE members SET password=? WHERE id=?', [bcrypt.hashSync(new_password, 10), req.params.id]);
    if (!r.rowCount) return res.status(404).json({ error: 'Member not found' });
...
router.get('/:id/score', authMiddleware, async (req, res, next) => {
  try {
    ...
    const savings  = await db.all('SELECT * FROM savings WHERE member_id=?', [req.params.id]);
    const loans    = await db.all('SELECT * FROM loans WHERE member_id=?', [req.params.id]);
...
router.get('/:id/activity', authMiddleware, async (req, res, next) => {
  try {
    if (req.user.role==='member' && req.user.id!==req.params.id) return res.status(403).json({ error:'Access denied' });
    const savings  = await db.all("SELECT 'saving' as type,id,month,amount,paid_date as date,status,penalty FROM savings WHERE member_id=? ORDER BY paid_date DESC LIMIT 10", [req.params.id]);
    const loans    = await db.all("SELECT 'loan' as type,id,amount,request_date as date,status,approve_date FROM loans WHERE member_id=? ORDER BY request_date DESC LIMIT 5", [req.params.id]);
    const repays   = await db.all(`SELECT 'repayment' as type,r.id,r.amount,r.paid_date as date,r.status,r.month,r.penalty FROM repayments r JOIN loans l ON r.loan_id=l.id WHERE l.member_id=? AND r.paid_date IS NOT NULL ORDER BY r.paid_date DESC LIMIT 10`, [req.params.id]);
...
router.get('/:id/stats', authMiddleware, adminOnly, async (req, res, next) => {
  try {
    const member = await db.one('SELECT * FROM members WHERE id=?', [req.params.id]);
    if (!member) return res.status(404).json({ error: 'Member not found' });
    const savings    = await db.all('SELECT * FROM savings WHERE member_id=? ORDER BY month ASC', [req.params.id]);
    const loans      = await db.all('SELECT * FROM loans WHERE member_id=? ORDER BY request_date DESC', [req.params.id]);
    const totalSaved = savings.filter(s=>['paid','late'].includes(s.status)).reduce((a,s)=>a+Number(s.amount),0);
    const totalPenalty = savings.reduce((a,s)=>a+Number(s.penalty||0),0);
    const activeLoan = loans.find(l=>l.status==='active');
    const repayments = activeLoan ? await db.all('SELECT * FROM repayments WHERE loan_id=? ORDER BY month ASC', [activeLoan.id]) : [];
...
router.post('/:id/exit', authMiddleware, adminOnly, async (req, res, next) => {
  try {
    ...
    const activeLoan = await db.one("SELECT id FROM loans WHERE member_id=? AND status='active'", [req.params.id]);
    if (activeLoan) return res.status(409).json({ error: 'Member has an active loan. Settle the loan before processing exit.' });
    const savRow = await db.one("SELECT COALESCE(SUM(amount),0) as t FROM savings WHERE member_id=? AND status IN ('paid','late')", [req.params.id]);
    const totalSavings = Number(savRow.t);
    const shareRefund  = Number(member.share_amount) || 1000;
    await db.run("UPDATE members SET status='exited',exit_reason=?,exit_date=?,exit_notes=? WHERE id=?",
...
router.get('/exits/list', authMiddleware, adminOnly, async (req, res, next) => {
  try {
    ...
    const result = await Promise.all(exits.map(async m => {
      const savRow = await db.one("SELECT COALESCE(SUM(amount),0) as t FROM savings WHERE member_id=? AND status IN ('paid','late')", [m.id]);
      const loanRow = await db.one("SELECT COUNT(*) as c FROM loans WHERE member_id=? AND status='active'", [m.id]);
