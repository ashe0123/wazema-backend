...
router.patch('/', authMiddleware, adminOnly, async (req, res, next) => {
  try {
    ...
    for (const [key, value] of Object.entries(updates)) {
      const r = await db.run("UPDATE settings SET value=?,updated_at=CURRENT_TIMESTAMP WHERE key=?", [String(value), key]);
...
router.post('/change-admin-password', authMiddleware, adminOnly, async (req, res, next) => {
  try {
    ...
    const admin = await db.one('SELECT * FROM admins WHERE id=?', [req.user.id]);
    if (!admin || !bcrypt.compareSync(current_password, admin.password)) return res.status(401).json({ error: 'Current password incorrect' });
    await db.run('UPDATE admins SET password=? WHERE id=?', [bcrypt.hashSync(new_password, 10), req.user.id]);
...
router.post('/announcements', authMiddleware, adminOnly, async (req, res, next) => {
  try {
    ...
    const r = await db.run("INSERT INTO announcements (title,body,priority,active) VALUES (?,?,?,1)", [title.trim(), body.trim(), validPriority]);
...
router.patch('/announcements/:id', authMiddleware, adminOnly, async (req, res, next) => {
  try {
    const ann = await db.one('SELECT * FROM announcements WHERE id=?', [req.params.id]);
    if (!ann) return res.status(404).json({ error: 'Announcement not found' });
    const { active, title, body, priority } = req.body;
    await db.run('UPDATE announcements SET title=?,body=?,priority=?,active=? WHERE id=?',
...
router.delete('/announcements/:id', authMiddleware, adminOnly, async (req, res, next) => {
  try {
    const r = await db.run('DELETE FROM announcements WHERE id=?', [req.params.id]);
...
router.get('/dividends', authMiddleware, async (req, res, next) => {
  try {
    if (req.user.role === 'member') {
      return res.json(await db.all('SELECT * FROM dividends WHERE member_id=? ORDER BY year DESC', [req.user.id]));
    }
    const year = req.query.year ? parseInt(req.query.year) : null;
    const sql  = year
      ? 'SELECT d.*,m.name as member_name FROM dividends d JOIN members m ON d.member_id=m.id WHERE d.year=? ORDER BY d.member_id ASC'
...
router.post('/dividends/distribute', authMiddleware, adminOnly, async (req, res, next) => {
  try {
    ...
    for (const m of members) {
      ...
      // Upsert
      const existing = await db.one('SELECT id FROM dividends WHERE member_id=? AND year=?', [m.id, year]);
      if (existing) {
        await db.run('UPDATE dividends SET share_qty=?,total_pool=?,member_share=?,status=? WHERE member_id=? AND year=?',
          [shares, Number(total_pool), memberShare, 'pending', m.id, year]);
      } else {
        await db.run('INSERT INTO dividends (member_id,year,share_qty,total_pool,member_share,status) VALUES (?,?,?,?,?,?)',
...
router.patch('/dividends/:id/pay', authMiddleware, adminOnly, async (req, res, next) => {
  try {
    ...
    const r = await db.run("UPDATE dividends SET status='paid',paid_date=? WHERE id=?", [paid_date || new Date().toISOString().split('T')[0], req.params.id]);
...
router.get('/reminders', authMiddleware, adminOnly, async (req, res, next) => {
  try {
    ...
    const unpaidSavings = await db.all(
      `SELECT m.id,m.name,m.phone,m.monthly_saving FROM members m WHERE m.status='active' AND NOT EXISTS (SELECT 1 FROM savings s WHERE s.member_id=m.id AND s.month=?)`,
      [cm]
    );
    const unpaidRepayments = await db.all(
      `SELECT r.*,m.name as member_name,m.phone as member_phone,l.id as loan_id FROM repayments r JOIN loans l ON r.loan_id=l.id JOIN members m ON l.member_id=m.id WHERE l.status='active' AND r.month=? AND r.status NOT IN ('paid','pending_review')`,
      [cm]
    );
  ...
  try {
    ...
    const unpaidSavings = await db.all(
      `SELECT m.id,m.name,m.phone,m.monthly_saving,
        CASE WHEN CAST(? AS INTEGER) > CAST(? AS INTEGER) THEN 'overdue' WHEN CAST(? AS INTEGER) > (CAST(? AS INTEGER) - 3) THEN 'due_soon' ELSE 'upcoming' END as urgency
       FROM members m WHERE m.status='active'
       AND NOT EXISTS (SELECT 1 FROM savings s WHERE s.member_id=m.id AND s.month=?)
       ORDER BY m.name ASC`,
      [dayOfMonth, savingsDueDay, dayOfMonth, savingsDueDay, cm]
    );
    ...
    const unpaidRepayments = await db.all(
      `SELECT r.*,m.name as member_name,m.phone as member_phone,l.id as loan_id,
       ...
       WHERE l.status='active' AND r.month=? AND r.status NOT IN ('paid','pending_review')
       ORDER BY m.name ASC`,
      [dayOfMonth, repayDueDay, dayOfMonth, repayDueDay, cm]
    );
...
router.get('/dashboard-stats', authMiddleware, adminOnly, async (req, res, next) => {
  try {
    ...
    const [tm, ts, tlb, cl, pl, sp, tps, tpr, cms, pms, ptm] = await Promise.all([
      ...
      db.one("SELECT COALESCE(SUM(amount),0) as t FROM savings WHERE month=? AND status IN ('paid','late')", [cm]),
      db.one("SELECT COALESCE(SUM(amount),0) as t FROM savings WHERE month=? AND status IN ('paid','late')", [prevMonth]),
      db.one("SELECT COUNT(*) as c FROM savings WHERE month=? AND status IN ('paid','late')", [cm]),
    ]);
    ...
    for (let i=5; i>=0; i--) {
      ...
      const row = await db.one("SELECT COALESCE(SUM(amount),0) as t,COUNT(*) as c FROM savings WHERE month=? AND status IN ('paid','late')", [mo]);
...
router.get('/financial-report', authMiddleware, adminOnly, async (req, res, next) => {
  try {
    ...
    const [tm, ts, tps, tpr, la, lc, tr, od, al] = await Promise.all([
      db.one("SELECT COUNT(*) as c FROM members WHERE status='active'"),
      db.one("SELECT COALESCE(SUM(amount),0) as t FROM savings WHERE status IN ('paid','late') AND paid_date BETWEEN ? AND ?", [fromDate, toDate]),
      db.one("SELECT COALESCE(SUM(penalty),0) as t FROM savings WHERE paid_date BETWEEN ? AND ?", [fromDate, toDate]),
      db.one("SELECT COALESCE(SUM(penalty),0) as t FROM repayments WHERE paid_date BETWEEN ? AND ?", [fromDate, toDate]),
      db.one("SELECT COUNT(*) as c, COALESCE(SUM(amount),0) as t FROM loans WHERE approve_date BETWEEN ? AND ?", [fromDate, toDate]),
      db.one("SELECT COUNT(*) as c FROM loans WHERE status='completed'"),
      db.one("SELECT COALESCE(SUM(amount),0) as t FROM repayments WHERE status='paid' AND paid_date BETWEEN ? AND ?", [fromDate, toDate]),
    ...
    const monthlyBreakdown = await db.all(
      `SELECT LEFT(paid_date,7) as month, COUNT(*) as payments, SUM(amount) as collected, SUM(penalty) as penalties FROM savings WHERE status IN ('paid','late') AND paid_date BETWEEN ? AND ? GROUP BY LEFT(paid_date,7) ORDER BY LEFT(paid_date,7) ASC`,
      [fromDate, toDate]
    );
...
router.post('/banks', authMiddleware, adminOnly, async (req, res, next) => {
  try {
    ...
    const r = await db.run(
      'INSERT INTO payment_banks (bank_name,account_number,account_holder,is_active,sort_order) VALUES (?,?,?,1,?)',
...
router.patch('/banks/:id', authMiddleware, adminOnly, async (req, res, next) => {
  try {
    const bank = await db.one('SELECT * FROM payment_banks WHERE id=?', [req.params.id]);
    ...
    if (account_number && !/^[\d\s\-]+$/.test(account_number.trim()))
      return res.status(400).json({ error: 'account_number must contain only digits, spaces, or dashes' });
    await db.run(
      'UPDATE payment_banks SET bank_name=?,account_number=?,account_holder=?,is_active=?,sort_order=? WHERE id=?',
      [
...
router.delete('/banks/:id', authMiddleware, adminOnly, async (req, res, next) => {
  try {
    const r = await db.run('DELETE FROM payment_banks WHERE id=?', [req.params.id]);
