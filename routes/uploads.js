...
router.post('/receipt', authMiddleware, upload.single('file'), async (req, res, next) => {
  ...
  try {
    ...
    if (type === 'saving') {
      const row = await db.one('SELECT * FROM savings WHERE id=?', [record_id]);
      ...
      await db.run('UPDATE savings SET receipt_url=? WHERE id=?', [url, record_id]);

    } else if (type === 'repayment') {
      const row = await db.one(
        'SELECT r.*,l.member_id FROM repayments r JOIN loans l ON r.loan_id=l.id WHERE r.id=?',
        [record_id]
      );
      ...
      await db.run('UPDATE repayments SET receipt_url=? WHERE id=?', [url, record_id]);
      ...
      if (type === 'guarantor') {
        const row = await db.one('SELECT * FROM loans WHERE id=?', [record_id]);
        if (!row) { fs.unlinkSync(uploadedPath); return res.status(404).json({ error: 'Loan not found' }); }
        await db.run('UPDATE loans SET guarantor_doc_url=? WHERE id=?', [url, record_id]);
      } else {
        const row = await db.one('SELECT * FROM members WHERE id=?', [record_id]);
        if (!row) { fs.unlinkSync(uploadedPath); return res.status(404).json({ error: 'Member not found' }); }
        await db.run(`UPDATE members SET ${colMap[type]}=? WHERE id=?`, [url, record_id]);
      }
    }
