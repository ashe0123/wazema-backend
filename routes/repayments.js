...
async function autoMarkOverdue(loan_id) {
  ...
  await db.run("UPDATE repayments SET status='overdue' WHERE loan_id=? AND status IN ('due','pending') AND month<?", [loan_id, cm]);
}
async function advanceQueue(loan_id) {
  const next = await db.one("SELECT * FROM repayments WHERE loan_id=? AND status='pending' ORDER BY month ASC LIMIT 1", [loan_id]);
  if (next) await db.run("UPDATE repayments SET status='due' WHERE id=?", [next.id]);
}
async function checkCompletion(loan_id, penalty, res) {
  const row = await db.one("SELECT COUNT(*) as c FROM repayments WHERE loan_id=? AND status NOT IN ('paid','refinanced')", [loan_id]);
  if (Number(row.c) === 0) {
    await db.run("UPDATE loans SET status='completed' WHERE id=?", [loan_id]);
  ...
  try {
    if (req.user.role === 'member') {
      return res.json(await db.all(
        `SELECT r.*,l.amount as loan_amount,l.id as loan_id FROM repayments r JOIN loans l ON r.loan_id=l.id WHERE l.member_id=? ORDER BY r.month ASC`,
        [req.user.id]
      ));
    }
    ...
    res.json(await db.all(
      `SELECT r.*,l.member_id,l.id as loan_id,m.name as member_name FROM repayments r JOIN loans l ON r.loan_id=l.id JOIN members m ON l.member_id=m.id WHERE l.status='active' AND r.month=? ORDER BY m.name ASC`,
      [month]
    ));
  } catch(e) { next(e); }
});
...
router.get('/overdue', authMiddleware, adminOnly, async (req, res, next) => {
  try {
    ...
    const rows = await db.all(
      `SELECT r.*,l.member_id,l.id as loan_id,m.name as member_name,m.phone as member_phone FROM repayments r JOIN loans l ON r.loan_id=l.id JOIN members m ON l.member_id=m.id WHERE l.status='active' AND r.status NOT IN ('paid','pending_review') AND r.month<? ORDER BY r.month ASC,m.name ASC`,
      [cm]
    );
...
router.get('/schedule/:loan_id', authMiddleware, async (req, res, next) => {
  try {
    const loan = await db.one('SELECT * FROM loans WHERE id=?', [req.params.loan_id]);
    ...
    const reps = await db.all('SELECT * FROM repayments WHERE loan_id=? ORDER BY month ASC', [req.params.loan_id]);
    const cm   = new Date().toISOString().slice(0,7);
...
router.post('/record', authMiddleware, async (req, res, next) => {
  try {
    ...
    const loan = await db.one('SELECT * FROM loans WHERE id=?', [loan_id]);
    if (!loan) return res.status(404).json({ error: 'Loan not found' });
    if (loan.status !== 'active') return res.status(400).json({ error: 'Loan is not active' });
    if (req.user.role==='member' && loan.member_id!==req.user.id) return res.status(403).json({ error: 'Access denied' });
    const rep = await db.one('SELECT * FROM repayments WHERE loan_id=? AND month=?', [loan_id, month]);
    ...
    if (req.user.role === 'member') {
      await db.run("UPDATE repayments SET status='pending_review',paid_date=?,penalty=?,bank_name=?,account_number=? WHERE loan_id=? AND month=?",
    ...
    await db.run("UPDATE repayments SET status='paid',paid_date=?,penalty=?,bank_name=?,account_number=? WHERE loan_id=? AND month=?",
...
router.post('/settle', authMiddleware, async (req, res, next) => {
  try {
    ...
    if (loan.status !== 'active') return res.status(400).json({ error: 'Loan is not active' });
    if (req.user.role==='member' && loan.member_id!==req.user.id) return res.status(403).json({ error: 'Access denied' });
    const unpaid = await db.all("SELECT * FROM repayments WHERE loan_id=? AND status NOT IN ('paid') ORDER BY month ASC", [loan_id]);
    ...
    if (req.user.role === 'member') {
      for (const r of unpaid) await db.run("UPDATE repayments SET status='pending_review',paid_date=? WHERE id=?", [paid_date, r.id]);
    ...
    for (const r of unpaid) await db.run("UPDATE repayments SET status='paid',paid_date=?,penalty=? WHERE id=?", [paid_date, settlePenalty/unpaid.length, r.id]);
    await db.run("UPDATE loans SET status='completed' WHERE id=?", [loan_id]);
  ...
  try {
    const row = await db.one(
      `SELECT r.*,l.member_id,l.amount as loan_amount,l.id as loan_id,m.name as member_name,m.phone as member_phone
       FROM repayments r JOIN loans l ON r.loan_id=l.id JOIN members m ON l.member_id=m.id WHERE r.id=?`,
      [req.params.id]
    );
...
router.patch('/:id/confirm', authMiddleware, adminOnly, async (req, res, next) => {
  try {
    const rep = await db.one('SELECT r.*,l.member_id,l.id as loan_id FROM repayments r JOIN loans l ON r.loan_id=l.id WHERE r.id=?', [req.params.id]);
    ...
    await db.run("UPDATE repayments SET status='paid',penalty=? WHERE id=?", [finalPenalty, rep.id]);
...
router.post('/bulk-confirm', authMiddleware, adminOnly, async (req, res, next) => {
  try {
    ...
    for (const id of ids) {
      try {
        const rep = await db.one('SELECT r.*,l.id as loan_id FROM repayments r JOIN loans l ON r.loan_id=l.id WHERE r.id=?', [id]);
        if (!rep) {
    ...
    for (const loanId of processedLoans) {
      try {
        ...
        const unpaid = await db.all("SELECT id FROM repayments WHERE loan_id=? AND status NOT IN ('paid','refinanced')", [loanId]);
        if (unpaid.length === 0) {
...
router.patch('/:id/waive-penalty', authMiddleware, adminOnly, async (req, res, next) => {
  try {
    const row = await db.one('SELECT * FROM repayments WHERE id=?', [req.params.id]);
    if (!row) return res.status(404).json({ error: 'Repayment not found' });
    await db.run('UPDATE repayments SET penalty=0 WHERE id=?', [req.params.id]);
