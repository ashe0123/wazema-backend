...
app.use(rateLimit({
  windowMs: 15 * 60 * 1000,
  max: isDev ? 5000 : (process.env.RATE_LIMIT_MAX ? parseInt(process.env.RATE_LIMIT_MAX) : 1000),
...
const authLimiter = rateLimit({
  windowMs: 15 * 60 * 1000,
  max: isDev ? 500 : (process.env.AUTH_RATE_LIMIT ? parseInt(process.env.AUTH_RATE_LIMIT) : 20),
  standardHeaders: true,
  legacyHeaders: false,
...
const sensitiveOpLimiter = rateLimit({
  windowMs: 60 * 60 * 1000,
  max: isDev ? 1000 : (process.env.SENSITIVE_OP_LIMIT ? parseInt(process.env.SENSITIVE_OP_LIMIT) : 100),
...
app.use('/api/financial-security',                require('./routes/financial-security'));
...
app.listen(PORT, async () => {
  ...
  // Initialize financial security systems
  try {
    const backup = require('./backup');
    const integrity = require('./integrity');
    
    console.log('  🔐 Initializing financial security systems...');
    ...
    if (process.env.BACKUP_ENABLED !== 'false') {
      ...
      console.log('  ⚠️  Automated backups disabled (set BACKUP_ENABLED=true to enable)');
    }
    
    // Start integrity checks
    if (process.env.INTEGRITY_CHECKS_ENABLED !== 'false') {
      integrity.scheduleIntegrityChecks();
      console.log('  ✅ Automated integrity checks enabled');
    } else {
      console.log('  ⚠️  Automated integrity checks disabled');
    }
    
    // Verify encryption configuration
    if (process.env.DATA_ENCRYPTION_KEY) {
      if (process.env.DATA_ENCRYPTION_KEY.length >= 32) {
        console.log('  ✅ Data encryption configured');
      } else {
        console.warn('  ⚠️  DATA_ENCRYPTION_KEY is too short (should be 32+ characters)');
      }
    } else {
      console.log('  ⚠️  Data encryption not configured (set DATA_ENCRYPTION_KEY)');
    }
    ...
    if (process.env.BACKUP_ENCRYPTION_KEY) {
      if (process.env.BACKUP_ENCRYPTION_KEY.length >= 32) {
        ...
        console.warn('  ⚠️  BACKUP_ENCRYPTION_KEY is too short (should be 32+ characters)');
      }
    } else {
      console.log('  ℹ️  Backup encryption not configured (backups will be unencrypted)');
    }
    
    console.log('');
  } catch (error) {
    console.error('  ❌ Failed to initialize financial security systems:', error.message);
  }

  // Check if any banks are configured
  try {
    const db = require('./db');
    const banks = await db.all("SELECT COUNT(*) as count FROM payment_banks WHERE is_active=1");
    if (banks[0] && banks[0].count === 0) {
      console.warn('  ⚠️  IMPORTANT: No bank accounts configured');
      console.warn('     Go to Admin → Settings → 🏦 Banks to add your bank account information.\n');
    } else if (banks[0] && banks[0].count > 0) {
      console.log(`  ✅ ${banks[0].count} bank account(s) configured\n`);
    }
  } catch {}
});
